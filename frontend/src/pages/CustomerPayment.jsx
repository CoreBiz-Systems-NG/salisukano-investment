...
const NewPayment = () => {
	...
	const { data, isLoading, error } = useQuery({
		...
		queryFn: async () => fetchCustomer({ token: user.token, id: customerId }),
	});
	...
	const handleSubmit = (e) => {
		...
		setDateError('');
		setIsLoading(true);
		const data = { customerId, name, total: amount, date, description };
		axios
			.post(`${apiUrl}/payments/customers`, data, config)
			.then((res) => {
				if (res.data) {
	...
	return (
		<>
			<main className="w-full py-3 pl-7 pr-5  flex flex-col space-y-3">
				...
				<div className="transform overflow-hidden rounded-2xl  p-4 bg-white text-left align-middle shadow-lg transition-all font-josefin">
